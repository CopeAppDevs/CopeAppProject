...
if not parameteres.dev:
    ...
    dbhost = options.get("dbhost", "localhost")
    dbport = options.get("dbport", "666")
    print("executing command: forever start -p "+realPath+"/data/logs/ app.js -p "+port+" --dev --dbhost "+dbhost+":"+dbport)
    ...
    subprocess.Popen("forever start -p "+realPath+"/data/logs/ app.js -p "+port+" --dev --dbhost "+dbhost+":"+dbport, shell=True)
    print('NODE STARTED!')
