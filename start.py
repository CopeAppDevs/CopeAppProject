...
if not parameteres.dev:
    ...
    dbhost = options.get("dbhost", "localhost")
    dbport = options.get("dbport", "666")
    print("executing command: forever start -l ./data/logs/"+port+".log -o ./data/logs/"+port+".out -e ./data/logs/"+port+".err -p ./data/forever/ app.js -p "+port+" --dev --dbhost "+dbhost+":"+dbport)
    ...
    subprocess.call("touch data/logs/node"+port+".out", shell=True)
    subprocess.call("touch data/logs/node"+port+".err", shell=True)
    subprocess.call("touch data/logs/node"+port+".log", shell=True)
