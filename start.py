...
if not parameteres.dev:
    ...
    dbhost = options.get("dbhost", "localhost")
    dbport = options.get("dbport", "666")
    print('executing command: # forever start -e '+port+'.err -o '+port+'.out -l '+port+'.log -p "'+realPath+'/data/logs/" "'+realPath+'/app.js" -p '+port+' --dev --dbhost '+dbhost+':'+dbport)
    ...
    subprocess.call('# forever start -e '+port+'.err -o '+port+'.out -l '+port+'.log -p "'+realPath+'/data/logs/" "'+realPath+'/app.js" -p '+port+' --dev --dbhost '+dbhost+':'+dbport, shell=True)
    print('NODE STARTED!')
